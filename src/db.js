...
function openDb(filePath) {
  ...
  const stmts = {
    insert: db.prepare(
      'INSERT INTO messages (id, body, created_at, ip, reply_to) VALUES (?, ?, ?, ?, ?)',
    ),
  ...
  return {
    ...
    insertMessage({ id, message, createdAt, ip, replyTo }) {
      stmts.insert.run(id, message, createdAt, ip, replyTo ?? null);
      return { id, message, createdAt, ip, replyTo: replyTo ?? null };
    },
