...
test('createLatestCache snapshots on creation and via manual refresh', () => {
  ...
  try {
    ...
    db.insertMessage({ id: uuidv7(), message: 'hi', createdAt: Date.now(), ip: '::1', poster: 'poster0000', replyTo: null });
...
test('createLatestCache refreshes automatically on its interval and stops on demand', async () => {
  ...
  try {
    db.insertMessage({ id: uuidv7(), message: 'auto', createdAt: Date.now(), ip: '::1', poster: 'poster0000', replyTo: null });
    await sleep(100);
    assert.equal(cache.get().messages.length, 1);

    cache.stop();
    db.insertMessage({ id: uuidv7(), message: 'after stop', createdAt: Date.now(), ip: '::1', poster: 'poster0000', replyTo: null });
    await sleep(100);
